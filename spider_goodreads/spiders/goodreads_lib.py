...
class LibrarySpider(scrapy.Spider):
    ...
    def parse(self, response):
        links = response.xpath(
            "//table[@id='libsresults']//p[@class='lib']/a/@href").extract()
        for link in links:
    ...
    def getInfo(self, response):
        ...
        title = response.xpath(
            libdata + "//h1/text()").extract_first().strip().replace("\n", "").replace("#1", "")
        psdata = response.xpath(libdata + "//p").extract()
        email = re.search(
            "mailto:(.*?)\"",
            response.body).group(1) if re.search(
            "mailto:(.*?)\"",
            response.body) else None
        website = response.xpath("//a[@class='lib-website']/@href").extract(
        )[0] if response.xpath("//a[@class='lib-website']/@href").extract() else None
        ...
        phone = re.search(
            "\(\d+\) \d+-\d+",
            response.body).group(0) if re.search(
            "\(\d+\) \d+-\d+",
            response.body) else None
        item = {}
        item["title"] = title
