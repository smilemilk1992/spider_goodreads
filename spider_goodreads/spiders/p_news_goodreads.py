...
class GoodReadsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        with open('cudos_goodreads.txt', "r") as f:
            ...
            for x in url:
                ...
                goodreadsReq = datas[1] + "." + \
                    "_".join(i for i in datas[2].split(" "))
    ...
    def parse(self, response):
        ...
        authorUrlList = []
        authorList = []
        for authorInfo in response.xpath(
                XpathRule.authorName__container).extract():
            authorInfo = etree.fromstring(authorInfo)
            greyText = authorInfo.xpath(
                ".//span[contains(@class,'greyText')]/text()")
            if greyText:
                if "Illustrator" in authorInfo.xpath(
                        "./span[contains(@class,'greyText')]/text()")[0].strip():
                    Tllist.append(
                        authorInfo.xpath("./a[@class='authorName']/@href")[0].strip())
                    Tlluser.append(
                        authorInfo.xpath("./a[@class='authorName']/span/text()")[0].strip())
                else:
                    authorUrlList.append(
                        authorInfo.xpath("./a[@class='authorName']/@href")[0].strip())
                    authorList.append(
                        authorInfo.xpath("./a[@class='authorName']/span/text()")[0].strip())
            else:
        ...
        description = "".join(
            x.strip() for x in response.xpath(
                XpathRule.description).extract()) if response.xpath(
            XpathRule.description).extract() else None
        bookFormat = response.xpath(
            XpathRule.bookFormat).extract()[0].strip() if response.xpath(
            XpathRule.bookFormat).extract() else None
        ...
        if "Original Title" in infoBoxRowTitle:
            Original_title = etree.fromstring(bookDataBox[infoBoxRowTitle.index(
                "Original Title")]).xpath("./text()")[0].strip()
        else:
            Original_title = None

        if "ISBN" in infoBoxRowTitle:
            ISBN = etree.fromstring(bookDataBox[infoBoxRowTitle.index("ISBN")]).xpath(
                "./text()")[0].strip()
            ISBN13 = \
            ...
            ISBN13 = etree.fromstring(
                bookDataBox[infoBoxRowTitle.index("ISBN13")]).xpath("./text()")[0].strip()
            ISBN = None
        else:
            ISBN = ISBN13 = None
        ...
        if "Literary Awards" in infoBoxRowTitle:
            Literary_Awards = ",".join(x for x in etree.fromstring(
                bookDataBox[infoBoxRowTitle.index("Literary Awards")]).xpath("./a[@class='award']/text()"))
        else:
        ...
        if len(details) >= 2:
            ...
            firstPublishedTime = etree.fromstring(
                details[1]).xpath(
                XpathRule.greyText)[0].strip().rstrip(")").lstrip("(") if etree.fromstring(
                details[1]).xpath(
                XpathRule.greyText) else publishedTime
        elif len(details) == 1:
            a = etree.fromstring(details[0]).xpath(
                "./text()")[0] if etree.fromstring(details[0]).xpath("./text()") else None
            publishedTime = "".join(
                x.strip() + " " for x in a.split("\n") if x)
            firstPublishedTime = etree.fromstring(
                details[0]).xpath(
        ...
        Rating_details = response.xpath(
            XpathRule.Rating_details).extract()[0].strip()
        ...
        for x in elementList:
            ...
            actionLinkLite = x.xpath(
                ".//a[@class='actionLinkLite bookPageGenreLink']/text()")
            if len(actionLinkLite) == 2:
                ...
                if actionLinkLite:
                    ...
                    a = None
            bookPageGenreLink = x.xpath(
                ".//a[@class='actionLinkLite greyText bookPageGenreLink']/text()")
        ...
        item["ISBN13"] = ISBN13.lstrip(
            "(ISBN13: ").rstrip(")") if ISBN13 else None
        item["genres"] = str(genres).replace("'", "\'") if genres else None
        item["bookFormat"] = bookFormat.replace(
            "'", "\'") if bookFormat else None
        item["publishedTime"] = publishedTime.replace(
            "'", "\'") if publishedTime else None
        item["firstPublishedTime"] = firstPublishedTime.replace(
            "'", "\'") if firstPublishedTime else None
        ...
        item["literaryAwards"] = Literary_Awards.replace(
            "'", "\'") if Literary_Awards else None
        item["editionLanguage"] = Edition_Language.replace(
            "'", "\'") if Edition_Language else None
        item['description'] = description.replace(
            "'", "\'").replace(
            "\n", "") if description else None

        otherEditionsActions = response.xpath(
            "//div[@class='otherEditionsActions']/a[@class='actionLinkLite']/@href").extract()
        if otherEditionsActions:
            actionLinkLite = "https://www.goodreads.com" + \
                otherEditionsActions[0]
    ...
    def otherLink(self, response):
        ...
        for info in infolist:
            ...
            for i in moreDetails:
                ...
                language = None
                for data in dataRow:
                    dataTitle = data.xpath(
                        "./div[@class='dataTitle']/text()")[0].strip()
                    if "Edition language" in dataTitle:
                        language = data.xpath(
                            "./div[@class='dataValue']/text()")[0].strip()
                    if "ISBN13" in dataTitle:
                        ISBN13 = data.xpath(
                            "./div[@class='dataValue']/text()")[0].strip().lstrip("(ISBN13: ").rstrip(")")
                        ISBN = None
                        # isbninfo[infoId]=[ISBN, ISBN13.lstrip("(ISBN13: ").rstrip(")")]
                    elif "ISBN" in dataTitle:
                        ISBN = data.xpath(
                            "./div[@class='dataValue']/text()")[0].strip()
                        greyText = data.xpath("./div[@class='dataValue']/span[@class='greyText']/text()")[0].strip(
                        ) if data.xpath("./div[@class='dataValue']/span[@class='greyText']/text()") else None
                        ISBN13 = greyText.lstrip(
                            "(ISBN13: ").rstrip(")") if greyText else None
